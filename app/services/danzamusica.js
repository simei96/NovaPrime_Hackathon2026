...
const FIRESTORE_COLLECTION = 'DanzaMusica';

function mapDanzaMusicaDoc(docSnap) {
...
// Category Chips
// nota: pendiente de campo de categoria (ver Categoria en mapDanzaMusicaDoc)
const CATEGORIAS_CHIPS = [
	{ label: 'Todos', icon: 'view-grid-outline' },
	{ label: 'Folclórica', icon: 'human-handsup' },
	{ label: 'Moderna', icon: 'music-note' },
	{ label: 'Talleres', icon: 'school-outline' },
...
// CTA Card tipo carrusel (próximas presentaciones)
const PROXIMAS_PRESENTACIONES = [
	{
		id: 'a-pre-1',
		titulo: 'Ballet Folclórico Nicaragüense',
		desc: 'Danzas tradicionales con trajes típicos de todo el país',
		fecha: 'Sábado 8:00 PM',
		imagen: 'https://images.unsplash.com/photo-1547153760-18fc86324498?auto=format&fit=crop&w=900&q=80',
	},
	{
		id: 'a-pre-2',
		titulo: 'Noche de Marimba',
		desc: 'Música en vivo en el corazón de Masaya',
		fecha: 'Viernes 7:00 PM',
		imagen: 'https://images.unsplash.com/photo-1465847899084-d164df4dedc6?auto=format&fit=crop&w=900&q=80',
	},
];

export default function DanzaMusica() {
	...
	// carga de datos
	const fetchDanzaMusica = async () => {
		try {
			setLoadError(false);
			const snap = await getDocs(collection(db, FIRESTORE_COLLECTION));
			const data = snap.docs.map(mapDanzaMusicaDoc);
			setItems(data);
		} catch (error) {
			console.error('Error cargando danza y música:', error);
			setLoadError(true);
		} finally {
			setLoading(false);
		}
	...
	useEffect(() => {
		const it = setInterval(() => setCarouselIndex((p) => (p + 1) % PROXIMAS_PRESENTACIONES.length), 3800);
		...
		<View key={item.id} style={styles.allCard}>
			...
			<View style={styles.allBody}>
				...
				<TouchableOpacity
					style={styles.allBtn}
					activeOpacity={0.85}
					onPress={() => router.push(`/screens/services/DanzaMusica/${item.id}`)}
				>
	...
	return (
		<View style={{ flex: 1, backgroundColor: '#f6fafd' }}>
			<Stack.Screen options={{ title: 'Danza y Música', headerTitleAlign: 'center' }} />
			<ScrollView
			...
			>
				...
				<View style={styles.hero}>
					{/* Decoraciones de danza y música nicaragüense */}
					...
					<Text style={styles.heroTitle}>Danza y Música</Text>
					<Text style={styles.heroSubtitle}>Ritmo, folclore y talento nicaragüense · Todo el año</Text>

					<View style={styles.searchRow}>
						<View style={styles.searchBox}>
							<TextInput
								style={styles.searchInput}
								placeholder="Buscar danza y música..."
				...
				{loading && (
					<View style={styles.loadingRow}>
						...
						<Text style={styles.loadingText}>Cargando danza y música...</Text>
					</View>
				)}
				...
				{!loading && loadError && (
					<View style={styles.emptyState}>
						...
						<TouchableOpacity style={styles.emptyBtn} activeOpacity={0.85} onPress={fetchDanzaMusica}>
					...
					<>
						...
						<View style={{ marginTop: 22 }}>
							<View style={styles.sectionHeaderRow}>
								...
								<Text style={styles.sectionTitle}>Próximas presentaciones</Text>
							</View>
							<ScrollView
								ref={scrollRef}
								horizontal
							...
							>
								{PROXIMAS_PRESENTACIONES.map((o, idx) => (
									<View key={o.id} style={[styles.offerCard, { marginRight: idx === PROXIMAS_PRESENTACIONES.length - 1 ? 0 : 14 }]}>
							...
							<View style={styles.dotsRow}>
								{PROXIMAS_PRESENTACIONES.map((_, i) => (
										...
										>
											...
											<View style={styles.peekFooterRow}>
												<Text style={styles.peekPrice}>
													{d.horario || 'Horario por confirmar'}
												</Text>
												<TouchableOpacity
													style={styles.peekBtn}
													activeOpacity={0.85}
													onPress={() => router.push(`/screens/services/DanzaMusica/${d.id}`)}
												>
						...
						<View style={{ marginTop: 26, paddingHorizontal: 16 }}>
							<View style={[styles.sectionHeaderRow, { paddingHorizontal: 0 }]}>
								...
								<Text style={styles.sectionTitle}>Toda la danza y música</Text>
							</View>
...
const styles = StyleSheet.create({
	...
	// CTA Card carrusel (próximas presentaciones)
	offerCard: { 
		width: 300,
		height: 190, 
		borderRadius: 8, 
