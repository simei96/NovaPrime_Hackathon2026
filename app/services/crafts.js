...
// Category Chips
// nota: pendiente de campo de categoria
const CATEGORIAS_CHIPS = [
	{ label: 'Todos', icon: 'view-grid-outline' },
	{ label: 'Cerámica', icon: 'palette-outline' },
	{ label: 'Textiles', icon: 'texture-box' },
	{ label: 'Joyería', icon: 'diamond-stone' },
...
// nota: pendiente de coleccion oferta
const OFERTAS = [
	{
		id: 'a-off-1',
		titulo: 'Descuento Feria',
		desc: '10% en compras mayores a C$ 1000',
		descuento: 10,
		precioNuevo: 900,
		precioAnterior: 1000,
		imagen: 'https://images.unsplash.com/photo-1509719662421-19a5d3c0aa9c?auto=format&fit=crop&w=900&q=80',
	},
	{
		id: 'a-off-2',
		titulo: 'Paquete Artesano',
		desc: 'Combo de 3 piezas seleccionadas',
		descuento: 15,
		precioNuevo: 510,
		precioAnterior: 600,
		imagen: 'https://images.unsplash.com/photo-1518998053901-5348d3961a04?auto=format&fit=crop&w=900&q=80',
	},
];
...
export default function Crafts() {
	...
	const fetchArtesanias = async () => {
		try {
			setLoadError(false);
			const snap = await getDocs(collection(db, 'Artesanias'));
			const data = snap.docs.map((docSnap) => {
				...
				return {
					...
					precio: typeof d.Precio === 'number' ? d.Precio : null,
			...
			console.error('Error cargando artesanías:', error);
			setLoadError(true);
		} finally {
			setLoading(false);
		}
		...
		<View key={item.id} style={styles.allCard}>
			...
			<View style={styles.allBody}>
				...
				<TouchableOpacity
					style={styles.allBtn}
					activeOpacity={0.85}
					onPress={() => router.push(`/screens/services/Artesanias/${item.id}`)}
				>
			...
			>
				...
				{loading && (
					<View style={styles.loadingRow}>
						...
						<Text style={styles.loadingText}>Cargando artesanías...</Text>
					</View>
				)}
				...
				{!loading && loadError && (
					<View style={styles.emptyState}>
						...
						<TouchableOpacity style={styles.emptyBtn} activeOpacity={0.85} onPress={fetchArtesanias}>
				...
				{!loading && !loadError && !filtroActivo && (
					<>
						...
						<View style={{ marginTop: 26 }}>
							...
							{destacados.length === 0 ? (
								<Text style={[styles.loadingText, { paddingHorizontal: 16 }]}>
									Aún no hay artesanías registradas.
								</Text>
							) : (
								<ScrollView
									horizontal
										...
										>
											...
											<View style={styles.peekFooterRow}>
												...
												<TouchableOpacity
													...
													activeOpacity={0.85}
													onPress={() => router.push(`/screens/services/Artesanias/${d.id}`)}
												>
						...
						{/* Todas las artesanías — grid con todos los documentos de Firestore */}
						<View style={{ marginTop: 26, paddingHorizontal: 16 }}>
							<View style={[styles.sectionHeaderRow, { paddingHorizontal: 0 }]}>
								...
								<Text style={styles.sectionTitle}>Todas las artesanías</Text>
							</View>
