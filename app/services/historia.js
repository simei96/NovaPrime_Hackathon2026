...
// PENDIENTE: cuando exista la colección "Historia" en Firestore,
...
const CATEGORIAS_CHIPS = [
	...
	{ label: 'Precolombino', icon: 'pyramid' },
	{ label: 'Colonial', icon: 'church-outline' },
	{ label: 'Independencia', icon: 'flag-outline' },
	{ label: 'Revolución', icon: 'star-shooting-outline' },
...
// nota: pendiente de colección de ofertas específica para Historia;
const OFERTAS = [
	{
		id: 'h-off-1',
		titulo: 'Tour Casco Histórico',
		desc: 'Recorrido guiado por el centro colonial',
		descuento: 10,
		precioNuevo: 450,
		precioAnterior: 500,
	...
	},
	{
		id: 'h-off-2',
		titulo: 'Museo + Guía',
		desc: 'Entrada combinada con visita guiada',
		descuento: 15,
		precioNuevo: 255,
		precioAnterior: 300,
		imagen: 'https://images.unsplash.com/photo-1552832230-c0197dd311b5?auto=format&fit=crop&w=900&q=80',
	},
];

// PENDIENTE FIRESTORE: mientras no exista la colección "Historia", se usan
// estos datos de ejemplo con exactamente la misma forma que tendrían los
// documentos reales (Nombre, Departamento, Horario, ImagenURL, Precio,
// Categoria, Rating), para que puedas migrar sin tocar el resto del
// componente cuando crees la colección.
const HISTORIA_MOCK = [
	{
		id: 'mock-1',
	...
	{
		id: 'mock-2',
		nombre: 'Fortaleza La Pólvora',
		departamento: 'Granada',
		horario: '9:00 AM - 4:00 PM',
		imagen: ['https://images.unsplash.com/photo-1524293581917-878a6d017c71?auto=format&fit=crop&w=900&q=80'],
		precio: 40,
		categoria: 'Colonial',
		rating: 4.5,
	},
	{
		...
		imagen: ['https://images.unsplash.com/photo-1553969723-2a0a1b0b5b0a?auto=format&fit=crop&w=900&q=80'],
		precio: 60,
	...
	{
		id: 'mock-4',
		nombre: 'Sitio Arqueológico Ometepe',
		departamento: 'Rivas',
		horario: '7:00 AM - 5:00 PM',
		imagen: [],
		precio: null,
		categoria: 'Precolombino',
		rating: 4.6,
	},
	{
		id: 'mock-5',
		nombre: 'Casa Museo Sandino',
		departamento: 'Masaya',
		horario: '9:00 AM - 4:00 PM',
		imagen: [],
		precio: 30,
...
export default function Historia() {
	...
	// PENDIENTE FIRESTORE: mientras no exista la colección "Historia", los
	// items arrancan con los datos mock de arriba en vez de un arreglo vacío.
	const [items, setItems] = useState(HISTORIA_MOCK);
	...
	const onRefresh = async () => {
		setRefreshing(true);
		// PENDIENTE FIRESTORE: cuando exista la colección, cambia esta línea
		// por: await fetchHistoria();
		...
		<View key={item.id} style={styles.allCard}>
			...
			<View style={styles.allBody}>
				...
				<TouchableOpacity
					style={styles.allBtn}
					activeOpacity={0.85}
					onPress={() => router.push(`/screens/services/historia/${item.id}`)}
				>
	...
	return (
		<View style={{ flex: 1, backgroundColor: '#f6fafd' }}>
			<Stack.Screen options={{ title: 'Historia', headerTitleAlign: 'center' }} />
			<ScrollView
			...
			>
				...
				<View style={styles.hero}>
					{/* Decoraciones temática histórica */}
					...
					<Text style={styles.heroSubtitle}>Sitios y museos históricos · Precios en córdobas</Text>
				...
				{loading && (
					<View style={styles.loadingRow}>
						...
						<Text style={styles.loadingText}>Cargando sitios históricos...</Text>
					</View>
				)}
				...
				{!loading && loadError && (
					<View style={styles.emptyState}>
						...
						<TouchableOpacity
							...
							activeOpacity={0.85}
							onPress={() => {
								// PENDIENTE FIRESTORE: reemplazar por fetchHistoria()
								setLoadError(false);
							}}
						>
				...
				{!loading && !loadError && !filtroActivo && (
					<>
						...
						<View style={{ marginTop: 26 }}>
							...
							{destacados.length === 0 ? (
								<Text style={[styles.loadingText, { paddingHorizontal: 16 }]}>
									Aún no hay sitios históricos registrados.
								</Text>
							) : (
								<ScrollView
									horizontal
									...
									{destacados.map((d, idx) => (
										...
										>
											...
											<Text style={styles.peekInfo} numberOfLines={1}>
												{d.departamento}{d.horario ? ` • ${d.horario}` : ''}
											</Text>
											<View style={styles.peekFooterRow}>
												<Text style={styles.peekPrice}>
													{d.precio != null ? `C$ ${d.precio}` : 'Consultar'}
												</Text>
												<TouchableOpacity
													style={styles.peekBtn}
													activeOpacity={0.85}
													onPress={() => router.push(`/screens/services/historia/${d.id}`)}
												>
						...
						{/* Todos los sitios históricos — grid con todos los documentos */}
						<View style={{ marginTop: 26, paddingHorizontal: 16 }}>
							<View style={[styles.sectionHeaderRow, { paddingHorizontal: 0 }]}>
								...
								<Text style={styles.sectionTitle}>Toda la historia</Text>
							</View>
										...
										<View style={styles.resultBody}>
											...
											<View style={styles.resultInfoRow}>
												...
												<Text style={styles.resultPrice} numberOfLines={1}>
													{item.precio != null ? `C$ ${item.precio}` : 'Consultar'}
												</Text>
											</View>
											<TouchableOpacity
